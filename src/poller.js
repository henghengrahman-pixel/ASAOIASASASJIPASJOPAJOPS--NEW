...
import { createVisibleReconciler } from './visible-reconciler.js';
...
let visibleReconciler=null,visibleReconcileLastAt=0,visibleReconcileRunning=false,visibleReconcileLast=null;
...
export function pollerStatus(){ return {running,paused,inFlight:running,lastTick,lastSuccess,lastError,lastResult,mode:config.lcSyncMode,pollMs:config.lcPollMs,trackedChats:summaryFingerprints.size,overlapPrevented:pollMetrics.overlapPrevented,workers:{...workerMetrics},visibleReconciler:visibleReconcileLast||visibleReconciler?.status?.()||null}; }
...
async function maybeRunVisibleReconciler(livechat,{force=false}={}){
  const now=Date.now();
  if(visibleReconcileRunning)return visibleReconcileLast;
  if(!force&&visibleReconcileLastAt&&(now-visibleReconcileLastAt)<config.lcVisibleReconcileMs)return visibleReconcileLast;
  visibleReconciler ||= createVisibleReconciler({db,livechat,batchSize:config.lcVisibleReconcileBatch,concurrency:config.lcVisibleReconcileConcurrency});
  visibleReconcileRunning=true;visibleReconcileLastAt=now;
  try{visibleReconcileLast=await visibleReconciler.run();return visibleReconcileLast;}
  catch(e){visibleReconcileLast={...(visibleReconciler.status?.()||{}),error:String(e?.message||e)};return visibleReconcileLast;}
  finally{visibleReconcileRunning=false;}
}
...
export async function syncOnce(livechat,{manual=false}={}){
  ...
  try{
    ...
    const visibleReconcile=await maybeRunVisibleReconciler(livechat);
    ...
    const inboxDelta=actualVisibleInbox-expectedVisibleInbox;
    const inboxAbsoluteMismatch=Math.abs(inboxDelta);
    const mismatchAuthoritative=Boolean(fullSweepReady);
    lastTick=new Date().toISOString();lastSuccess=lastTick;
    lastResult={ok:true,listSource:data?._listSource||'unknown',pagesFetched:Number(data?._pageCount||1),rawChats:inventory.length,deduplicatedChats:inventory.length,myActiveChats:providerMyActiveChats,providerMyActiveChats,activeFromProvider:providerMyActiveChats,closedFromProvider,archivedFromProvider,expectedVisibleInbox,actualVisibleInbox,visibleInboxActual:actualVisibleInbox,visibleInbox:actualVisibleInbox,discoveryChats:chats.length,enqueued,unchanged,inventorySize:Number(data?._inventorySize||inventory.length),inventoryComplete:data?._inventoryComplete!==false,deepSyncRunning:Boolean(data?._deepSyncRunning),deepGeneration:deepGen,deepCompletedGeneration:completedGeneration,deepLastSuccess:data?._deepLastSuccess||null,deepLastError:data?._deepLastError||data?._deepSyncError||null,closedHiddenThisCycle,archivedHiddenThisCycle,providerTerminalSeen:closedFromProvider+archivedFromProvider,providerTerminalLocalMatch:Number(visibleReconcile?.terminalLocalMatch||0),providerTerminalAlreadyClosed:Number(visibleReconcile?.terminalAlreadyClosed||0),providerTerminalHiddenNow:Number(visibleReconcile?.terminalHiddenNow||0)+closedHiddenThisCycle+archivedHiddenThisCycle,providerTerminalNotLocal:Number(visibleReconcile?.terminalNotLocal||0),visibleReconciler:visibleReconcile||null,reconcileCandidates:Number(reconcile?.candidates||0),reconciledClosed:Number(reconcile?.reconciled||0),reconciledRestored:0,invalidInboxRepaired:invalidRepaired,staleResponseRejected:runtimeMetrics.staleResponseRejected,send403RequesterNotUser:runtimeMetrics.send403RequesterNotUser,endChatAttempts:runtimeMetrics.endChatAttempts,endChatSuccess:runtimeMetrics.endChatSuccess,endChatFailure:runtimeMetrics.endChatFailure,endChatLastStatus:runtimeMetrics.endChatLastStatus,endChatLastProviderMessage:runtimeMetrics.endChatLastProviderMessage,endChatLastPayloadKeys:runtimeMetrics.endChatLastPayloadKeys,endChatLastChatIdSafe:runtimeMetrics.endChatLastChatIdSafe,queueDepth:Number(qm.depth||0),oldestJobAgeMs:Number(qm.oldest_job_age_ms||0),paginationDurationMs:Date.now()-started,durationMs:Date.now()-started,fetchErrors:0,fetchTimeouts:pollMetrics.fetchTimeouts,inboxDelta,inboxAbsoluteMismatch,mismatchAuthoritative,inboxMismatch:inboxAbsoluteMismatch};
    await db.setIntegrationHealth('livechat_discovery',{status:inboxAbsoluteMismatch===0||!mismatchAuthoritative?'OK':'WARNING',latencyMs:Date.now()-started,error:inboxAbsoluteMismatch===0?'':`${inboxAbsoluteMismatch} inbox lifecycle rows differ from provider${mismatchAuthoritative?'':' (inventory partial; non-authoritative)'}`,meta:lastResult}).catch(()=>{});
