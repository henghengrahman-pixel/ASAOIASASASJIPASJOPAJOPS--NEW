...
import { endConversationByRouteId } from './conversation-lifecycle-service.js';
...
async function assertConversationSendable(chatId){
  ...
  if(!lifecycle||lifecycle.visible_in_inbox===false||['closed','archived'].includes(String(lifecycle.status||'').toLowerCase())){const e=new Error('CANCELLED_CHAT_TERMINAL');e.status=409;e.code='CANCELLED_CHAT_TERMINAL';e.legacyCode='LIVECHAT_SEND_BLOCKED_CLOSED_OR_HIDDEN';throw e;}if(lifecycle.lc_send_blocked===true){const e=new Error('LIVECHAT_SEND_BLOCKED_ACCESS_STATE');e.status=409;throw e;}
  return lifecycle;
}
async function handleRequesterNotUser(chatId,error){
  ...
  let refresh=null;
  try{const state=await lc.getChatLifecycle(chatId);refresh=state.lifecycle;if(refresh.isClosed||refresh.isArchived)await closeConversationFromLiveChat(chatId,'SEND_403_PROVIDER_TERMINAL',{...refresh,archived:refresh.isArchived});else await markConversationSendBlocked(chatId,'REQUESTER_NOT_USER_ACTIVE');}catch(refreshError){await logError('livechat_send','LIFECYCLE_REFRESH_AFTER_403_FAILED',refreshError.message,{chatId}).catch(()=>{});}
...
app.post('/api/conversations/:id/end',requireAdmin,async(req,res)=>{
