...
async function refreshLifecycleAfterSend403(livechat,chatId){
  try{
    ...
    await db.markConversationSendBlocked(chatId,'REQUESTER_NOT_USER_ACTIVE');
    return {terminal:false,sendBlocked:true,lifecycle};
  }catch(refreshError){
...
async function sendAndStore(livechat,chatId,text,intent,senderType='ai'){
  ...
  if(!lifecycle || lifecycle.visible_in_inbox===false || ['closed','archived'].includes(String(lifecycle.status||'').toLowerCase())){
    const er=new Error('CANCELLED_CHAT_TERMINAL');er.status=409;er.code='CANCELLED_CHAT_TERMINAL';er.legacyCode='LIVECHAT_SEND_BLOCKED_CLOSED_OR_HIDDEN';throw er;
  }
  if(lifecycle.lc_send_blocked===true){const er=new Error('LIVECHAT_SEND_BLOCKED_ACCESS_STATE');er.status=409;er.code='LIVECHAT_SEND_BLOCKED_ACCESS_STATE';throw er;}
