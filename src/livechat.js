...
import { recordEndChatMetric } from './livechat-metrics.js';
...
export class LiveChatClient {
  ...
  async getChatLifecycle(chatId) {
    const id=String(chatId||'').trim();
    if(!id){const er=new Error('LIVECHAT_CHAT_ID_REQUIRED');er.status=400;throw er;}
    const data=await this.call('get_chat',{chat_id:id});
  ...
  async endChat(chatId) {
    ...
    const chatIdSafe=`${id.slice(0,6)}…${crypto.createHash('sha256').update(id).digest('hex').slice(0,10)}`;
    try{
      const response=await this.call('deactivate_chat',body);
      recordEndChatMetric({ok:true,status:200,payloadKeys,chatIdSafe});
      console.info(JSON.stringify({event:'livechat_end_chat',action:'deactivate_chat',payloadKeys,chatId:chatIdSafe,status:200,ok:true}));
      return {ok:true,requestShape:'id',payloadKeys,response};
    }catch(error){
      const status=Number(error?.status)||502;
      const providerMessage=String(error?.data?.error?.message||error?.message||'unable to deactivate chat');
      recordEndChatMetric({ok:false,status,providerMessage,payloadKeys,chatIdSafe});
      console.warn(JSON.stringify({event:'livechat_end_chat',action:'deactivate_chat',payloadKeys,chatId:chatIdSafe,status,providerError:providerMessage.slice(0,300),ok:false}));
      const er=new Error(`LIVECHAT_END_FAILED: ${providerMessage}`);
      er.status=status;er.cause=error;er.data=error?.data;er.action='deactivate_chat';er.payloadKeys=payloadKeys;er.chatIdSafe=chatIdSafe;
      throw er;
    }
  }
